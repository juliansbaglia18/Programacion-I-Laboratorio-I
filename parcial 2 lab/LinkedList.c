...
/** \brief Incrementa en 1 el tamaño de la lista
 *
 * \param list LinkedList* la lista a incrementar
...
/** \brief Decrementa el tamaño de la lista
 *
 * \param this LinkedList* lista a decrementar
 * \return int -1 si el puntero es NULL, 0 si pudo hacerlo.
 *
 */
 ...
 *          e inicializa el tamaño en 0 y que el primer nodo sea NULL
 *
 *  \param void
...
Node* ll_get_lNode(LinkedList* this)
{
    ...
    if (this != NULL && ll_isEmpty(this) == 0)//isEmpty verifica que la lista no este vacia
    {
 ...
 *  (creo que es para agregar un nuevo nodo)
 * \param this Node* nodo a setear
 * \param nextNode Node* proximo nodo
 * \return int 0 si this y nextNode son NULL, 1 si lo seteo
 *
 */
...
static Node* getNode(LinkedList* this, int nodeIndex)
{
    ...
    {
        if (nodeIndex < (ll_len(this)))//verifica que el indice dado sea menor a la cantidad de elementos
        {
            int actualIndex = 0;
            Node* auxNode = NULL;
...
/** \brief Setea el elemento de un nodo
 *
 * \param this Node* Nodo
 * \param ptrElement void* Elemento
...
{
    int returnAux = -1;
    if (this != NULL && nodeIndex > (-1))//verifica que this sea valido y  que el indice sea 0 o mas
    {
        if (nodeIndex <= ll_len(this)) //verifica que el indice dado sea menor o igual a la cantidad de elementos de la lista
        {
            Node* auxNode = ll_get_fNode(this);
            Node* prevNode = auxNode;//crea dos nodos iguales al primer elemento de la lista por parametro
            int actualIndex = 0;
            while (auxNode != NULL && actualIndex < nodeIndex)//busca el lugar en donde se quiere poner el nuevo elemento
            {
                prevNode = auxNode;
                auxNode = ll_get_nNode(auxNode);
                actualIndex++;
            }
            if (nodeIndex == 0)//si el indice por parametro es 0, se quiere agregar un primer elemento
            {
                // entonces no entra en el while y entraria aca
                if (auxNode != NULL)
                {
                    Node* newNode = ll_newNode(pElement);
                    ll_set_fNode(this, newNode); //agrega el nuevo nodo en el primer lugar
                    ll_set_NextNode(newNode, auxNode); //pone el anterior primer elemento en el segundo lugar
                    ll_increase_size(this); //incrementa el tamaño
                    returnAux = 0;
                }
                else//sino hace lo mismo pero no agrega nada en el segundo lugar ya que era una
                {
                    //lista vacia y el nuevo elemento es el unico
                    Node* newNode = ll_newNode(pElement);
            ...
            else if (actualIndex==nodeIndex)//agrega el elemento en donde indica el indice (cuando es mayor a 0)
            {
                Node* newNode = ll_newNode(pElement);
                ll_set_NextNode(prevNode,newNode);//ubica el nuevo nodo despues del nodo anterior
                ll_set_NextNode(newNode,auxNode);//ubica los siguientes nodos despues del nuevo nodo
                ll_increase_size(this);
                returnAux = 0;
            }
        }
...
int ll_add(LinkedList* this, void* pElement)
{
    ...
    if (this!=NULL)
    {
        ...
        {
        case 0://si la lista tiene elementos, agrega el nuevo elemento al final de la lista
            ...
            break;
        case 1://si esta vacia, le agrega el nuevo elemento en el primer lugar
            if (newNode != NULL)
            {
                ll_set_fNode(this,newNode);
                ll_increase_size(this);
...
/** \brief Devuelve el elemento contenido por el nodo señalado
 *
 * \param this LinkedList* Puntero a la lista
...
/** \brief elimina el primer elemento y pone el segundo elemento como primero
 *
 * \param this LinkedList* lista con los elementos a eliminar
 * \return int -1 si no lo pudo realizar. 1 si pudo
 *
 */
...
/** \brief Inserta un nuevo elemento en la lista en la posicion indicada
            Hace lo mismo que addNode pero el return no es estatico
 *
 * \param this LinkedList* Puntero a la lista
...
/** \brief Elimina el elemento de la posicion indicada y retorna lo que contiene
 *
 * \param this LinkedList* Puntero a la lista
...
/** \brief Pasa por toda la lista y le realiza la funcion pasada por parametro
 *
 * \param this LinkedList* lista
 * \param *pfunc int funcion a realizar
 * \return int -1 si hubo un error. Sino, lo que devuelva la funcion
 *
 */
 ...
 * \return LinkedList NULL si no funciono correctamente
                      newList
 *
 */
LinkedList* ll_filter(LinkedList* this, int (*pFunc)(void*,char*))
{
    ...
    if (this != NULL && pFunc != NULL)
    {
        ...
        void* temp = NULL;
        int i;
        getString("Ingrese el genero: ", temp2);

        for (i=0; i < ll_len(this); i++)
        {
            temp = ll_get(this,i);
            if (pFunc(temp,temp2) == 1)
            {
                ll_add(newList,temp);
            }
            temp = NULL;
